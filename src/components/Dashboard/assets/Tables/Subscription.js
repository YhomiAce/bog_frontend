...
import { deleteSubscriptionPlan } from '../../../../redux/actions/SubscriptionAction';
...
export function SubscriptionTable({openEditModal}) {
  ...
  const deletePlan = (id) => {
    Swal.fire({
      title: "Delete Plan",
      text: 'Do you want to delete this Subscription Plan?',
      icon: 'warning',
      ...
      },
      {
        ...
        Cell: (row) => <Menu placement="left-start" className="w-16">
          ...
          <MenuList className="w-16 bg-gray-100 fw-600 text-black">
            <MenuItem onClick={() => openEditModal(row.row.original)}>Edit Plan</MenuItem>
            <MenuItem 
            className="bg-red-600 hover:bg-red-500 hover:text-white text-white"
            onClick={()=> deletePlan(row.value)}
